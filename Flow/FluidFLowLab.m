...
DiameterVenturi2Small=.85*.0254;
DiameterVenturi2Large=2.065*.0254;
...
% From http://www.engineeringtoolbox.com/minor-loss-coefficients-pipes-d_626.html
% Assume Elbow, Flanged Long Radius 45 degrees
Kelbow=.2;
KelbowError=0;
...
%% Plots Volume Flow Rate Comparison
...
for i=1:length(p1)
    ...
    wazzup(i)=cell2mat(QExperimental1{i}(1,3));
end
Plot_Reynolds=errorbar(x,y,wazzup,'r*');
ylabel('Benchmark Flow [m^{3}/sec]')
xlabel('Venturi Meter Flow [m^{3}/sec]')
grid minor
grid on
...
Plot_Reynolds_Fit=fplot(@(x) fit(1).*x+fit(2), [0 max(x)]);
...
legend([Plot_hff_verus_v Plot_hfc_verus_v Laminar Transitional],{'Darcy-Weisbach Friction Factor','Hazen-William Friction Factor','Laminar End','Transitional End'},'Location','NorthWest');
grid minor
grid on
hold off
...
legend([Plot_hff_verus_logv Plot_hfc_verus_logv Laminar Transitional],{'Darcy-Weisbach Friction Factor','Hazen-William Friction Factor','Laminar End','Transitional End'},'Location','NorthWest');

hold off
saveas(3,'HeadLossesLog.png')
...
% Volume Flow Rate [m^3/sec]
for i=1:length(ElbowStraightUpstream)
    QTheoreticalelbow{i}=PropError(QinTheoryCalibrated,[A1 A2 P1 P2 rho slope intercept],[AreaVenturi2Large{1} AreaVenturi2Small{1} ElbowVenturiUpstream(i) ElbowVenturiDownstream(i) density fit(1) fit(2)],[AreaVenturi2Large{1,3} AreaVenturi2Small{1,3} PressureError PressureError desnityError 0 0]);
end

% Velocity [m/sec]
for i=1:length(ElbowStraightUpstream)
    VExperimentalelbow{i}=PropError(Velocity,[volumeflowrate A],[QTheoreticalelbow{i}{1,1} AreaPipe{1,1}],[QTheoreticalelbow{i}{1,3}  AreaPipe{1,3}]);
end

% Reynold's Number
for i=1:length(ElbowStraightUpstream)
    ReynoldExperimentalelbow{i}=PropError(Reynold,[Vel D rho MU],[VExperimentalelbow{i}{1,1} DiameterPipe density mu],[VExperimentalelbow{i}{1,3}  DiameterError desnityError muError]);
end

% Head Loss Pressure [m]
for i=1:length(ElbowStraightUpstream)
    HeadLosselbowPressure{i}=PropError(HeadChange,[P1 P2 specificweight],[ElbowStraightUpstream(i) ElbowStraightDownstream(i) gamma],[PressureError PressureError gammaError]);
end

% Head Loss Darcy-Weisbach [m]
for i=1:length(ElbowStraightUpstream)
    HeadLossfelbowline{i}=PropError(headlossf,[L d g v F],[ElbowLength DiameterPipe gravity VExperimentalelbow{i}{1,1} FFactor],[ElbowLengthError DiameterError gravityerror VExperimentalelbow{i}{1,3} FFactorError]);
end

% Head Loss Hazen-William [m]
for i=1:length(ElbowStraightUpstream)
    HeadLosscelbowline{i}=PropError(headlossc,[L v k c R],[ElbowLength VExperimentalelbow{i}{1,1} kconversion CFactor HydraulicRadius{1,1}],[ElbowLengthError VExperimentalelbow{i}{1,3} kError CFactorError HydraulicRadius{1,3}]);
end

% Elbow Loss Individual
for i=1:length(ElbowStraightUpstream)
    HeadLosselbowindividual{i}=PropError(helbow,[k g v],[Kelbow gravity VExperimentalelbow{i}{1,1}],[KelbowError gravityerror VExperimentalelbow{i}{1,3}]);
end

% Head Loss Darcy-Weisbach Total [m]
for i=1:length(ElbowStraightUpstream)
    HeadLossfelbowtotal{i}=PropError(htotal,[n he hl],[4 HeadLosselbowindividual{i}{1,1} HeadLossfelbowline{i}{1,1}],[0 HeadLosselbowindividual{i}{1,3} HeadLossfelbowline{i}{1,3}]);
end

% Head Loss Hazen-William Total [m]
for i=1:length(ElbowStraightUpstream)
    HeadLosscelbowtotal{i}=PropError(htotal,[n he hl],[4 HeadLosselbowindividual{i}{1,1} HeadLosscelbowline{i}{1,1}],[0 HeadLosselbowindividual{i}{1,3} HeadLosscelbowline{i}{1,3}]);
end

% Plots
for i=1:length(ElbowStraightUpstream)
    p(i)=cell2mat(VExperimentalelbow{i}(1,1));
    o(i)=cell2mat(HeadLossfelbowtotal{i}(1,1));
    u(i)=cell2mat(HeadLosscelbowtotal{i}(1,1));
    w(i)=cell2mat(HeadLosselbowPressure{i}(1,1));
    pE(i)=cell2mat(VExperimentalelbow{i}(1,3));
    oE(i)=cell2mat(HeadLossfelbowtotal{i}(1,3));
    uE(i)=cell2mat(HeadLosscelbowtotal{i}(1,3));
    wE(i)=cell2mat(HeadLosselbowPressure{i}(1,3));
end
...
% Darcy Friction Friction Factor
Plot_hff_verus_v_elbow=plot(p,o,'r*');

% Hazen-William Friction Factor
Plot_hfc_verus_v_elbow=plot(p,u,'b*');

% Pressure Drop
Plot_hfpressure_verus_v_elbow=plot(p,w,'g*');

ylabel('Head Loss [m]')
xlabel('Velocity [m/sec]')
legend([Plot_hff_verus_v_elbow Plot_hfc_verus_v_elbow Plot_hfpressure_verus_v_elbow],{'Darcy Friction Factor','Hazen-William Friction Factor','Pressure Drop'},'Location','NorthWest');
grid minor
grid on
hold off
saveas(4,'HeadLossesElbow.png')


% k factor solution
for i=1:length(ElbowStraightUpstream)
    kfactorsattained{i}=PropError(kcoefficientfactor,[hl g v],[HeadLosselbowPressure{i}{1,1} gravity VExperimentalelbow{i}{1,1}],[HeadLosselbowPressure{i}{1,3} gravityerror VExperimentalelbow{i}{1,3}]);
end

%% Uncertainty Graph

for i=1:15
    ValueT(i)=ReynoldTheoretical1{i}(1,1);
    ValueTE(i)=ReynoldTheoretical1{i}(2,3);
    ValueE(i)=ReynoldExperimental1{i}(1,1);
    ValueEE(i)=ReynoldExperimental1{i}(2,3);
end
figure
plot((cell2mat(ValueT(1:15))),(cell2mat(ValueTE(1:15))),'*')
grid minor
grid on
xlabel('Reynolds Number')
ylabel('% Uncertainty')
saveas(5,'VenturiUn.png')

figure
plot((cell2mat(ValueE(1:15))),(cell2mat(ValueEE(1:15))),'*')
grid minor
grid on
xlabel('Reynolds Number')
ylabel('% Uncertainty')
saveas(6,'CylinderUn.png')

ReynoldTheoretical1

ReynoldExperimental1